...
export class NPC extends Phaser.Physics.Arcade.Sprite {
  ...
  constructor(scene: Phaser.Scene, x: number, y: number, data: NpcData) {
    const tex = scene.textures.exists('char_neutral') ? 'char_neutral' : '__DEFAULT';
    super(scene, x, y, tex, 0);
    // ustawiamy texture po konstrukcji, bo getTextureKey używa this.faction
    ...
    if (scene.textures.exists(key)) this.setTexture(key, 0);
    this.setOrigin(0.5, 0.9);
    this.setDepth(y);
    this.setCollideWorldBounds(true);
    this.setDrag(1000);
    const body = this.body as Phaser.Physics.Arcade.Body;
    if (body) body.setMaxSpeed(120);
    const dw = Math.min(16, this.displayWidth * 0.4);
    const dh = Math.min(22, this.displayHeight * 0.4);
    body.setSize(dw, dh);
    body.setOffset((this.displayWidth - dw)/2, this.displayHeight - dh);

    // Etykieta imienia nad głową (osobny obiekt, podąża za NPC w update)
    const labelColor = this.faction === 'old_order' ? '#ffdd66'
      : this.faction === 'new_order' ? '#ff8844'
      : this.faction === 'bandit' ? '#aa4040'
      : '#cccccc';
    this.nameLabel = scene.add.text(x, y - this.height - 8, data.name, {
      fontSize: '10px', color: labelColor, stroke: '#000', strokeThickness: 3, fontStyle: 'bold'
    }).setOrigin(0.5).setDepth(y+1);

    this.setInteractive(new Phaser.Geom.Rectangle(-16, -this.height, 32, this.height), Phaser.Geom.Rectangle.Contains);
    this.on('pointerover', () => this.setTint(0xffff88));
  ...
  private loadSchedules() {
    const gameData = (this.scene as GameScene).gameData;
  ...
  update(delta: number, player: Player, timeSystem?: TimeSystem) {
    // Podążanie labeli za NPC
    this.nameLabel.setPosition(this.x, this.y - this.height - 6);
    ...
    if (this.isHostile) {
      ...
      if (d > 28) {
        ...
        if (!(this as any).lastAttack || now - (this as any).lastAttack > 1200) {
          ...
          if (!player.isAlive) (this.scene as GameScene).onPlayerDeath();
        }
      }
      // Obrót w stronę gracza
      if (player.x < this.x) { this.setFrame(1); this.setFlipX(false); }
      else { this.setFrame(2); this.setFlipX(true); }
      return;
    }
    ...
    if (this.moveTarget) {
      ...
      if (d < 6) {
        ...
        if (Math.abs(Math.cos(angle)) > Math.abs(Math.sin(angle))) {
          if (Math.cos(angle) < 0) { this.setFrame(1); this.setFlipX(false); }
          else { this.setFrame(2); this.setFlipX(true); }
        } else {
          this.setFrame(Math.sin(angle) < 0 ? 3 : 0);
        }
      }
    } else {
      if (Math.random() < 0.01) {
        this.moveTarget = { x: this.home.x + Phaser.Math.Between(-30, 30), y: this.home.y + Phaser.Math.Between(-30, 30) };
      } else {
        const b = this.body as Phaser.Physics.Arcade.Body;
        if (b && Math.abs(b.velocity.x) < 1 && Math.abs(b.velocity.y) < 1) b.setVelocity(0,0);
      }
    }
  ...
  private followSchedule(timeSystem?: TimeSystem) {
    ...
    if (pos) this.moveTarget = { x: pos.x * 2, y: pos.y * 2 };
  }

  private showDialogBubble(text: string, duration: number = 2000) {
    if (this.dialogBubble) this.dialogBubble.destroy();
    this.dialogBubble = this.scene.add.container(this.x, this.y - this.height - 20);
  ...
  interact(scene: GameScene) {
    ...
    if (this.npcData.is_trainer || this.npcData.teaching) {
      ...
      if (trainer) { this.openTrainerDialog(scene, trainer); return; }
    }
    ...
    scene.scene.launch('DialogScene', { npcData: this.npcData, gameScene: scene });
  }
  ...
  private openTrainerDialog(scene: GameScene, trainer: any) {
    ...
    trainer.skills.forEach((sk: any) => {
      const cur = scene.player.skillRanks[sk.skill_id] || 0;
      const text = `${sk.skill_name} (${cur}/${sk.max_rank}) - ${sk.cost_per_rank.gold}zł, ${sk.cost_per_rank.skill_points}pn`;
      ...
      rect.on('pointerdown', () => {
        ...
        if (res.ok) { cont.destroy(); scene.scene.resume('GameScene'); }
      });
      y += 36;
    });
    ...
    closeRect.on('pointerdown', () => { cont.destroy(); scene.scene.resume('GameScene'); });
    cont.setDepth(10000);
  }
  ...
  private openJoinDialog(scene: GameScene, faction: 'old_order'|'new_order') {
    ...
    cont.add(scene.add.text(0,-25,'Decyzja jest ostateczna.',{fontSize:'10px',color:'#cccccc'}).setOrigin(0.5));
    ...
    yes.on('pointerdown',()=>{scene.joinFaction(faction);cont.destroy();scene.scene.resume('GameScene');});
    no.on('pointerdown',()=>{cont.destroy();scene.scene.resume('GameScene');});
    cont.setDepth(10000);
  }
